...
def train(X_train, X_test, Y_train, Y_test, batch_size, drop_rate, lr, n_epochs, n_neurons, reg_rate, verbose):
    ...
    history = model.fit(X_train, Y_train, validation_data=[X_test, Y_test], epochs=n_epochs, verbose=0, batch_size=batch_size)
...
def main():
    X_val = np.load('data/outputs/flf_zero_padding/val_data.npy')
    X_test = np.load('data/outputs/flf_zero_padding/test_data.npy')
    errors_val = np.load('data/outputs/flf_zero_padding/val_errors.npy')
    errors_test = np.load('data/outputs/flf_zero_padding/test_errors.npy')
    preds_val = np.load('data/outputs/flf_zero_padding/val_preds.npy')
    preds_test = np.load('data/outputs/flf_zero_padding/test_preds.npy')
    Y_val = np.load('data/outputs/flf_zero_padding/val_labels.npy')
    Y_test = np.load('data/outputs/flf_zero_padding/test_labels.npy')
    ...
    # # clf = SVR(epsilon=0.001, C=0.25)
    # clf.fit(X_val.reshape(-1, 376), errors_val[:,0])
    # preds_errors_aro_val = clf.predict(X_val.reshape(-1, 376))
    # preds_errors_aro_test = clf.predict(X_test.reshape(-1, 376))
    # print('\n\n')
    ...
    # # clf = SVR(epsilon=0.001, C=0.25)
    # clf.fit(X_val.reshape(-1, 376), errors_val[:,1])
    # preds_errors_vale_val = clf.predict(X_val.reshape(-1, 376))
    # preds_errors_vale_test = clf.predict(X_test.reshape(-1, 376))
    # print('\n\n')
    ...
    clf.fit(X_val, errors_val)
    preds_errors_val = clf.predict(X_val)
    preds_errors_test = clf.predict(X_test)
    ...
    # model.save('data/models/errors_flf_zero_padding/lstm_rnn_'+str(round(loss,4))+'_'+str(batch_size)+'_'+str(drop_rate)+'_'+str(lr)+'_'+str(n_epochs)+'_'+str(n_neurons)+'_'+str(reg_rate)+'.h5')
    ...
    print("std errors val relative error:", (np.linalg.norm(np.std(errors_val, axis=0)-np.std(preds_errors_val, axis=0),2)/np.linalg.norm(np.std(errors_val, axis=0), 2)))
    ...
    print("std errors test relative error:", (np.linalg.norm(np.std(errors_test, axis=0)-np.std(preds_errors_test, axis=0),2)/np.linalg.norm(np.std(errors_test, axis=0), 2)))
