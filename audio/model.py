...
from utils import standarize, emotion_to_id, fuse_arousal_valence_data
...
class Model():    
    def train_continuous(self, X, y, dataset):
        if(dataset == 'mosei'):
            ...
            y_aro_train = (y_aro_train[idxs_aro_val]-1)/4-1
            X_val_train = X_val_train[idxs_aro_val]
            y_val_train = (y_val_train[idxs_aro_val]-1)/4-1
            ...
            y_aro_test = (y_aro_test[idxs_aro_val]-1)/4-1
            X_val_test = X_val_test[idxs_aro_val]
            y_val_test = (y_val_test[idxs_aro_val]-1)/4-1
            ...
            np.save('data/'+dataset+'/preds_labels.npy', y_preds_all)
        elif(dataset == 'hci'):
            ...
            for i in range(len(X_emo)):
                if(not np.isnan(X_emo[i]).any()):
                    idxs_emo.append(i)
                if(not np.isnan(X_aro[i]).any()):
                    idxs_aro.append(i)
                if(not np.isnan(X_val[i]).any()):
                    idxs_val.append(i)
            X_emo = X_emo[idxs_emo]
            y_emo = y_emo[idxs_emo]
            X_aro = X_aro[idxs_aro]
            y_aro = (y_aro[idxs_aro]-1)/4-1
            X_val = X_val[idxs_val]
            y_val = (y_val[idxs_val]-1)/4-1
            ...
            # emotion
            X_emo_train, X_emo_test, y_emo_train, y_emo_test = train_test_split(X_emo, y_emo, test_size=0.3)
            clf = RandomForestClassifier(n_estimators=100)
            clf.fit(X_emo_train, y_emo_train)
            acc = clf.score(X_emo_test, y_emo_test)
            print('accuracy for emotions:', acc)
            ...
            # arousal
            X_aro_train, X_aro_test, y_aro_train, y_aro_test = train_test_split(X_aro, y_aro, test_size=0.3)
            clf = RandomForestRegressor(n_estimators=100)
            clf.fit(X_aro_train, y_aro_train)
            r2_aro = clf.score(X_aro_test, y_aro_test)
            print('R2 for arousal:', r2_aro)
            ...
            preds_train = (preds_train-1)/2-1
            preds_test = (preds_test-1)/2-1
            ...
            y_emo = y_emo[idxs_emo]
            X_aro = X_aro[idxs]
            y_aro = (y_aro[idxs]-1)/2-1
            X_val = X_val[idxs]
            y_val = (y_val[idxs]-1)/2-1
